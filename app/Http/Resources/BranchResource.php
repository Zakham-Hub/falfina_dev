...
class BranchResource extends JsonResource
{
    public function toArray($request)
    {
        return [
            ...
            'daily_schedules' => $this->dailySchedules->map(function($schedule) {
                return [
                    'id' => $schedule->id,
                    'day' => $schedule->day,
                    ...
                    'shifts' => $schedule->shifts->map(function($shift) {
                        return [
                            'id' => $shift->id,
                            'name' => $shift->name,
                            'delivery_start_time' => $shift->delivery_start_time,
            ...
            'exceptional_holidays' => $this->exceptionalHolidays->map(function($holiday) {
                return [
                    'id' => $holiday->id,
                    'title' => $holiday->title,
            ...
            'special_occasions' => $this->specialOccasions->map(function($occasion) {
                return [
                    'id' => $occasion->id,
