...
<div id="kt_content_container" class="container-xxl">
    <div class="mb-5 card card-xxl-stretch mb-xl-8">
        ...
        <div class="py-3 card-body">
            <form action="{{ route('admin.branches.store') }}" method="POST" id="branch-form">
                @csrf
                ...
                <div class="row">
                    ...
                    <div class="col-md-3">
                        ...
                        <input type="text" id="coordinate" name="coordinate" class="form-control" value="{{ old('coordinate') }}">
                    </div>
                </div>
                ...
                <div class="mb-3 mt-3">
                    ...
                    <input type="text" id="search" class="form-control" placeholder="اكتب اسم الشارع أو المنطقة" value="{{ old('address') }}">
                </div>
                            ...
                            <div class="col-md-2">
                                <div class="form-check form-switch">
                                    <input class="form-check-input holiday-switch" type="checkbox"
                                           ...
                                           id="holiday-{{ $day }}"
                                           @if(old("daily_schedules.$day.is_holiday")) checked @endif>
                                    <label class="form-check-label" for="holiday-{{ $day }}">إجازة</label>
                                </div>
                            </div>

                            <div class="col-md-10 shifts-container" id="shifts-container-{{ $day }}">
                                @if(old("daily_schedules.$day.shifts"))
                                    @foreach(old("daily_schedules.$day.shifts") as $index => $shift)
                ...
                <button type="submit" class="mt-5 btn btn-success w-100">حفظ الفرع</button>
            </form>
        </div>
    </div>
</div>
 ...
 $('#copy-schedule-btn').click(function() {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                // Get the shifts HTML before any changes
                ...
                toDays.forEach(day => {
                    ...
                    const targetSwitch = targetDiv.find('.holiday-switch');
                    const targetContainer = targetDiv.find('.shifts-container');

                    // Update holiday switch state
                    targetSwitch.prop('checked', isHoliday).trigger('change');

                    if (!isHoliday) {
                        // Clear existing shifts
                        targetContainer.empty();

                        // Clone each shift row individually to avoid reference issues
                        sourceDiv.find('.shift-row').each(function() {
                            const clonedRow = $(this).clone();
                            targetContainer.append(clonedRow);
                        });

                        // Reindex the shifts for the target day
                        targetContainer.find('.shift-row').each(function(index) {
                            $(this).find('input').each(function() {
                                // Replace the source day with target day in the name attribute
                                const newName = $(this).attr('name')
                                    .replace(new RegExp(`\\[${fromDay}\\]\\[shifts\\]`), `[${day}][shifts]`)
                                    .replace(/\[\d+\]/, `[${index}]`);
                                $(this).attr('name', newName);
                            });
                        });

                        // If no shifts exist after copying, add one automatically
                        if (targetContainer.find('.shift-row').length === 0) {
                            targetDiv.find('.add-shift-btn').trigger('click');
                        }
                    }
                });
    ...
    let holidayCounter = {{ count(old('exceptional_holidays', [])) }};
    ...
    let occasionCounter = {{ count(old('special_occasions', [])) }};
...
<script>
    ...
    function initMap() {
        // Set initial position from old input or default to Cairo
        const initialLat = {{ old('latitude', '30.0444') }};
        const initialLng = {{ old('longitude', '31.2357') }};
        ...
        // If we have an address from old input, set it
        @if(old('address'))
            document.getElementById("search").value = "{{ old('address') }}";
        @endif
    }
